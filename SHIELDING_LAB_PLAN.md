...
| 8 | LIVE data readout (Phase 2 of live mode) | ✅ `live-context.js` — observed Kp (1-min) + Kyoto Dst quicklook at T2 cadence as CONTEXT (never drivers): G-scale + storm-phase words on the verdict card; live-drivers panel charts the ring buffer on the wall clock (`DriverChart` — NOW cursor, the forecast-lead window shaded, Bz median-filtered as fed to the solver with By dashed, Dst 24 h); Boyle 1997 empirical CPCP (`boyleCpcpKv` — deliberately unsaturated) rides the CPCP chart as the dashed live reference. Node gate: `tests/shielding-live-context.mjs`. |
