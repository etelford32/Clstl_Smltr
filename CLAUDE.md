...
| Ionosphere / M-I coupling layer (ring-current.html: efield bars, teardrop plasmapause, airglow + bubbles) | `IONOSPHERE_EXPLORATION_PLAN.md` (tracks + status), then `js/ring-current-efield.js` + `js/ionosphere-fountain.js` headers (pure kernels — run `node tests/ring-current-efield.mjs` + `node tests/ionosphere-fountain.mjs` after edits; the Maynard–Chen A(Kp) in efield MIRRORS `ring-current-transport.js` `convectionAmplitude` — change together). Rendering in `js/ring-current-ionosphere.js`; browser smoke `tests/ring-current-iono-smoke.spec.js`. |
