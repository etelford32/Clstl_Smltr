...
| Moon page (moon.html: radiation lab, interior cutaway, exosphere, landmarks, sky mode + eclipse calendar) | `js/moon-interior-model.js` + `js/moon-exosphere-model.js` + `js/moon-ephemeris.js` headers — PURE kernels (structure/tides/paleo-dynamo; species table/Jeans escape/live sodium weather; truncated Meeus ephemeris + libration + ch. 54 eclipse engine); the renderers draw ONLY kernel numbers. The Moon has NO collisional atmosphere and NO living dynamo — the page says so on purpose: the "atmosphere" layer is a collisionless exosphere and the dynamo scrubber is a memorial with per-epoch mechanisms. Sky mode orients + lights the globe from the ephemeris (sub-Earth = libration, sub-solar = terminator; ONE phase clock — don't re-add a synodic approximation) and the eclipse calendar is COMPUTED, pinned to the canonical catalog by its gate. Landmark data `js/moon-landmarks-data.js` pins IAU coords + the Reiner Gamma cross-link to `CRUSTAL_ANOMALIES` (one place, two views). Run `node tests/moon-interior-model.mjs tests/moon-exosphere-model.mjs tests/moon-landmarks.mjs tests/moon-ephemeris.mjs` after kernel/data edits; browser gates `tests/moon-interior-smoke.spec.js` + `tests/moon-surface-smoke.spec.js` (hovers a marker via the `window.__moonLab` hook) |
