...
const OUT_PATH = root('data/shielding-verdict-config.json');

// Threshold derivation from measured percentiles — kept in ONE place so
// `--rethreshold` (re-derive from the committed config's recorded
// percentiles, no 35-min replay) can never drift from a fresh bake.
// Entry thresholds are NOISE-anchored, not tier-anchored: 3× the quiet
// 95th percentile clears flapping headroom above the noise floor while
// staying below the watch tier (p75) — with the phase-aware buckets the
// measured floor is tiny, so a big fixed minimum would sit above the
// tiers and make them dead on entry (the 2026-07-24 second-bake wart).
function deriveConfig(pcts, driftQuietRatio) {
    const round = (x, d = 4) => (x == null ? null : Number(x.toFixed(d)));
    const nf = pcts.quiet_abs_epen_p95_mvpm;
    return {
        percentiles: {
            storm_abs_epen_p75_mvpm: round(pcts.storm_abs_epen_p75_mvpm),
            storm_abs_epen_p90_mvpm: round(pcts.storm_abs_epen_p90_mvpm),
            storm_abs_epen_p98_mvpm: round(pcts.storm_abs_epen_p98_mvpm),
            quiet_abs_epen_p95_mvpm: round(nf),
        },
        tiers: {
            watch_mvpm: round(pcts.storm_abs_epen_p75_mvpm),
            moderate_mvpm: round(pcts.storm_abs_epen_p90_mvpm),
            strong_mvpm: round(pcts.storm_abs_epen_p98_mvpm),
        },
        thresholds: {
            under_e_mvpm: round(Math.max(3 * nf, 0.003)),
            over_e_mvpm: round(-Math.max(0.75 * 3 * nf, 0.002)),
        },
        drift_quiet_ratio: round(driftQuietRatio, 3),
    };
}

// ── --rethreshold: recompute derived values from the committed config ──
if (process.argv.includes('--rethreshold')) {
    const prev = JSON.parse(await readFile(OUT_PATH, 'utf8'));
    const derived = deriveConfig(prev.percentiles, prev.drift_quiet_ratio);
    const config = { ...prev, ...derived, rethresholded_at: new Date().toISOString() };
    if (![config.tiers.watch_mvpm, config.thresholds.under_e_mvpm].every(Number.isFinite)) {
        console.error('committed config has non-finite percentiles — run a full bake instead.');
        process.exit(1);
    }
    await writeFile(OUT_PATH, JSON.stringify(config, null, 2) + '\n');
    console.log(`re-derived thresholds from recorded percentiles → ${OUT_PATH}`);
...
const config = {
    _comment: 'GENERATED by scripts/calibrate-shielding-verdict.mjs — do not hand-edit. '
        + 'Severity tiers are driven-interval |E_pen| percentiles (75/90/98); state entry '
        + 'thresholds are 3× the pre-storm quiet-phase 95th-percentile noise floor. All from '
        + 'the committed hindcast replay(s) below. Regenerate when the Gannon 2024 bundle bakes '
        + '(or `--rethreshold` to re-derive without a replay). Model-scale values, not absolute '
        + 'geophysical mV/m (40° boundary readout — see the page\'s known-simplifications section).',
    generated_at: new Date().toISOString(),
    provenance,
    ...deriveConfig({
        storm_abs_epen_p75_mvpm: p75,
        storm_abs_epen_p90_mvpm: p90,
        storm_abs_epen_p98_mvpm: p98,
        quiet_abs_epen_p95_mvpm: noiseFloor,
    }, driftQuietRatio),
};
