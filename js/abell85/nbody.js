...
/** Core radius r_c such that thinning ρ by r²/(r²+r_c²) removes ≈ mDeficit. */
export function coreRadiusForDeficit(host, rMax, mDeficit) {
    if (mDeficit <= 0) return 0;
    const removed = (rc) => {
        let sum = 0;
        const nSteps = 60;
        for (let k = 0; k < nSteps; k++) {
            const r = rMax * Math.pow((k + 0.5) / nSteps, 2);
            const dr = rMax * (Math.pow((k + 1) / nSteps, 2) - Math.pow(k / nSteps, 2));
...
/**
 * Deterministic cluster initial conditions written INTO caller-owned arrays
 * (plain Float32Arrays or views into WASM linear memory — same code path for
 * both engines). Returns the carved core radius used.
 */
export function sampleClusterInto(sc, rMax, n, seed, mDeficit, tangentialBias, pos, vel, flags) {
    const rand = mulberry32(seed);
    const rc = coreRadiusForDeficit(sc.host, rMax, mDeficit);
    const host = sc.host;
...
/** Star inspector record — shared by the JS and WASM cluster wrappers. */
export function starStateOf({ pos, vel, flags, lLc, sc }, i, mBh = 0) {
    const j = i * 3;
    ...
    const eSpec = 0.5 * v * v + sc.host.phi(r) - (mBh > 0 ? G * mBh / r : 0);
    return { i, r, v, L, eSpec, flag: flags[i], lRatio: lLc > 0 ? L / lLc : NaN };
}
...
export class StarCluster {
     ...
     *
     * Per-star adaptive substepping: the 0.05 Myr substep exists to resolve
     * star–hole encounters and cusp orbits; the outer majority of stars have
     * dynamical times ≫ dtMyr and one KDK step per frame is already accurate,
     * so full resolution is spent only near the holes / the cusp (tier radii
     * scale with r_infl). The tier is chosen from frame-start geometry and the
     * BHs are frozen for the frame, so per-star results are independent of
     * iteration order — and the WASM kernel (rust-abell85) implements the
     * IDENTICAL rule and float32 store/reload sequence: the engines must stay
     * bit-exact (tests/abell85-physics.mjs). Change one, change both.
     */
    step(dtMyr, bh1, m1, bh2, m2) {
        const nFull = Math.min(Math.max(Math.ceil(dtMyr / 0.05), 1), 12);
        const nMid = Math.ceil(nFull / 2);
        const rFast2 = (0.15 * this.sc.rInfl) ** 2;
        const rMid2 = (0.5 * this.sc.rInfl) ** 2;
        const { pos, vel, flags, n } = this;
        for (let i = 0; i < n; i++) {
            const j = i * 3;
            if (flags[i] === 2) {           // frozen ejecta: one pure drift
                pos[j] += vel[j] * dtMyr * KMS_MYR;
                pos[j + 1] += vel[j + 1] * dtMyr * KMS_MYR;
                pos[j + 2] += vel[j + 2] * dtMyr * KMS_MYR;
                continue;
            }
            const x = pos[j], y = pos[j + 1], z = pos[j + 2];
            let d2 = x * x + y * y + z * z;
            if (m1) {
                const dx = bh1[0] - x, dy = bh1[1] - y, dz = bh1[2] - z;
                d2 = Math.min(d2, dx * dx + dy * dy + dz * dz);
            }
            if (m2) {
                const dx = bh2[0] - x, dy = bh2[1] - y, dz = bh2[2] - z;
                d2 = Math.min(d2, dx * dx + dy * dy + dz * dz);
            }
            const nSub = d2 < rFast2 ? nFull : d2 < rMid2 ? nMid : 1;
            this._kdkStar(i, nSub, dtMyr / nSub, bh1, m1, bh2, m2);
        }
    }
    ...
    /** All substeps of ONE star for this frame (KDK + ejection bookkeeping). */
    _kdkStar(i, nSub, h, bh1, m1, bh2, m2) {
        const acc = this._tmp || (this._tmp = new Float64Array(3));
        const { pos, vel, flags } = this;
        const j = i * 3;
        for (let s = 0; s < nSub; s++) {
            ...
            if (flags[i] === 0 || flags[i] === 3) {
                ...
                if (r2 > R_FREEZE * R_FREEZE) { flags[i] = 2; break; }
            }
        }
    }
